...
datacsv3 <- read.csv("./t3.csv")
datacsv12 <- full_join(datacsv1, datacsv2, by = "ID")
datacsv <- full_join(datacsv12, datacsv3, by = "ID")
N <- nrow(datacsv)
...
colnames(pcorr) <- c(paste0("T1", c("Y2", "Y3", "ALL")), paste0("T2", c("Y2", "Y3", "ALL")), paste0("T3", c("Y2", "Y3", "ALL")))
rownames(pcorr) <- c(paste0("(", 1:I, ")"), "All")

complete_indices <- !is.na(X[,1,1]) & !is.na(X[,1,2]) & !is.na(X[,1,3])
...
ipar_mcmc_lhodina <- cbind(ipar_mcmc_lhodina_mean[,1], ipar_mcmc_lhodina_sd[,1], ipar_mcmc_lhodina_mean[,2], ipar_mcmc_lhodina_sd[,2], ipar_mcmc_lhodina_mean[,3], ipar_mcmc_lhodina_sd[,3], ipar_mcmc_lhodina_mean[,4], ipar_mcmc_lhodina_sd[,4], ipar_mcmc_lhodina_mean[,5], ipar_mcmc_lhodina_sd[,5], ipar_mcmc_lhodina_mean[,6], ipar_mcmc_lhodina_sd[,6])
colnames(ipar_mcmc_lhodina) <- c("guessing T1", "SD", "guessing T2", "SD", "guessing T3", "SD", "slipping T1", "SD", "slipping T2", "SD", "slipping T3", "SD")
...
theta_mcmc_lhodina <- cbind(theta_mcmc_lhodina_mean[,1], theta_mcmc_lhodina_sd[,1], theta_mcmc_lhodina_mean[,2], theta_mcmc_lhodina_sd[,2], theta_mcmc_lhodina_mean[,3], theta_mcmc_lhodina_sd[,3])
colnames(theta_mcmc_lhodina) <- c("T1", "SD1", "T2", "SD2", "T3", "SD3")
...
t3_pat <- c("000", "100", "010", "001", "110", "101", "011", "111", pat_table[complete_indices,3])
...
xtable_13 <- table(t1_pat, t3_pat)
diag(xtable_13) <- diag(xtable_13) - 1
xtable_13 <- cbind(xtable_13, table(t1_pat) - 1)
xtable_13 <- rbind(xtable_13, colSums(xtable_13))
colnames(xtable_13) <- c(colnames(xtable_13)[1:8], "SUM1")
rownames(xtable_13) <- c(rownames(xtable_13)[1:8], "SUM3")

xtable_23 <- table(t2_pat, t3_pat)
diag(xtable_23) <- diag(xtable_23) - 1
xtable_23 <- cbind(xtable_23, table(t2_pat) - 1)
xtable_23 <- rbind(xtable_23, colSums(xtable_23))
colnames(xtable_23) <- c(colnames(xtable_23)[1:8], "SUM2")
rownames(xtable_23) <- c(rownames(xtable_23)[1:8], "SUM3")
